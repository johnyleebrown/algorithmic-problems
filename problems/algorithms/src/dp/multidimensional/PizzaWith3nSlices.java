...
public class PizzaWith3nSlices {
		...
		private int dfs(int step, int curSum,int wholeSum,int[] a,
			...
			if (memo[step][maxind][wholeSum]!=null) {
				...
				for (int i = lo; i < hi; i++) {
					...
					int l = findLeft(i); int r = findRight(i);
	...
	class Solution3 {
		...
		private int dfs(int step, int curSum, int[] a,
		                Map<Integer, Integer> m,int lo,int hi) {
			// System.out.println(step);
			if (step == n / 3) {
				return curSum;
			}
			...
			for (int i = lo; i < hi; i++) {
				...
				int r = findRight(i);
				seen[r] = true;
				s.add(i);s.add(l);s.add(r);
				int ans = dfs(step + 1, curSum + a[i], a,m,lo,hi);
				s.remove(i);s.remove(l);s.remove(r);
				ret=Math.max(ret, ans);
				seen[i] = seen[l] = seen[r] = false;
			}
			m.put(h,ret);
	...
	public static void main(String[] args) {
		new Tester(new Solution0())
				.add(new int[]{10,1,1,2,1,10,3,10,2,8,4,10,8,8,2,9,9,9,10,10,7,6,5,6}).expect(63)
				.run();
	}
}

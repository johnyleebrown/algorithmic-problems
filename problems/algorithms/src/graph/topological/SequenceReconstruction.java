...
public class SequenceReconstruction {
    /**
     * We need to find a topsort, compare it with org and verify if it is
     * unique.
     ...
     *
     * Edge cases:
     * - when one of seqs has repeating values
     */
    public static class Solution {
        ...
        private boolean haveEdges(List<Integer> res, Map<Integer, Set<Integer>> g) {
            for (int i = 0; i < res.size() - 1; i++) {
                if (!g.get(res.get(i)).contains(res.get(i + 1)) && !g.get(res.get(i + 1)).contains(res.get(i))) {
                    return false;
                }
            }
            return true;
        ...
        private void dfs(int v, Map<Integer, Set<Integer>> g, List<Integer> res, Set<Integer> seen) {
        ...
        private boolean hasCycles(int v, Map<Integer, Set<Integer>> g, Set<Integer> globalSeen, Set<Integer> localSeen) {

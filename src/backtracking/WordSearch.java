...
{
	// check if we can construct the word starting from every letter 
	// and moving to its adj cells
	// so use dfs
	class Solution {
		...
		public boolean exist(char[][] board, String word) {
			...
			for (int i = 0; i < board.length; i++) for (int j = 0; j < board[0].length; j++) {
				...
				board[i][j] = ' ';
				if (generate(t, 0, String.valueOf(t), i, j, board, word)) return true;
				board[i][j] = t;
			}
				...
				int j, char[][] board, String word)
		{
			...
			for (int[] dir: dirs)
			{
				...
				board[newI][newJ] = ' ';
				if (generate(temp, len + 1, cur + temp, newI, newJ, board, word)) return true;

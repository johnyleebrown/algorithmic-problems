...
public class StoneGameII {
	/**
	 * every time we want to return the maximum we can get from the player
	 * it means that every player plays optimally
	 * and we want to minimize their results from various indexes
	 * so since the strategy is the same for both players
	 * => recurence function gon be same as well
	 * so recurrence gon be smth like
	 * find min of next player
	 * min = Math.min(min, helper(a, i+x, Math.max(M,x)));
	 * our maximum is sum of the part to the right minus their minimum:
	 * dp(i,M) = sums[i] - min
	 */

...
public class JumpGame {
  ...
  /**
   * For each index we create intervals for jump [min, max]. For each index we
   * are computing if new interval intersects with the biggest interval so far
   * (main interval). If not then we can't get to the new index from any of the
   * prev positions. Draw the picture with intervals to understand.
   */

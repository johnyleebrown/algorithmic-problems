 ...
 * Example 1:
 *
 * Input: nums = [1,5,11,5] Output: true Explanation: The array can be partitioned as [1, 5, 5] and
 * [11]. Example 2:
 *
 * Input: nums = [1,2,3,5] Output: false Explanation: The array cannot be partitioned into equal sum
 * subsets.
 *
 *
 * Constraints:
...
public class PartitionEqualSubsetSum {
  ...
  /**
   * Recursive Top-down Improved. You can spot that we always make a call to ind + 1 and none other.
   * So we don't really need to keep those parameters because we linearly move towards the end.
   */
  ...
  /**
   * Bottom-up Iterative Optimized Using 1d Here we go from right to left because we want to reuse
   * the answers from previous iteration, otherwise we will rewrite them. And also we are going
   * until we hit i == a[j] because other numbers will result in sum < 0.
   */

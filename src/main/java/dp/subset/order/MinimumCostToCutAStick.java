...
public class MinimumCostToCutAStick {
	/**
	 * State:
	 * dp[i] (dp[lo][hi]) - min cost if we cut subarray [lo,hi] at some position
	 *
	 * Subproblem:
	 * Get cost of cutting if we will cut subarray [lo,hi] at index k.
	 *
	 * Idea:
	 * We try to cut at different indexes, then try to cut subarrays [lo,k] and [k,hi]
	 *
	 * Recurrence:
	 * dp[lo][hi] = Math.min(curLength + minCostLeft + minCostRight, dp[lo][hi])
	 * where curLength = hi-lo
	 */
	public static class Solution {
		public int minCost(int n, int[] ar) {
			if (ar.length == 1) return n;

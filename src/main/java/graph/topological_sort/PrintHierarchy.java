...
public class PrintHierarchy {
	...
	public static class Solution {
		...
		private boolean hasCycle(Map<String, List<String>[]> g) {
			Set<String> globalSeen = new HashSet<>();
			for (String v : g.keySet()) {
				if (hasCycleHelper(v, g, globalSeen, new HashSet<>())) {
					return true;
				}
			}
			return false;
		...
		private boolean hasCycleHelper(String v, Map<String, List<String>[]> g,
			...
			if (g.get(v)[0] != null) {
				for (String w : g.get(v)[0]) {
					if (hasCycleHelper(w, g, globalSeen, localSeen)) {
						return true;
					}
				}
			}

    ...
    for (int i = 0; i < profitsList.size(); i++) {
      ...
      for (Knapsack.Solution s : solutions) {
        Future<?> f = executor.submit(() -> {
          ...
          s.solveKnapsack(profitsList.get(finalI), weightsList.get(finalI),
              capacityList.get(finalI));

          long t = System.nanoTime() - startTime;

          ansStrings.add(
              "Test " + (finalI + 1) + ". Time for " + s.getClass().getSimpleName() + " is " + t
                  + " ns");
